...
def get_station_info(station_id: int, stations: List["Station"]) -> list:
    ...
    for item in stations:
        if item[ID] == station_id:
            information.append(item[NAME])
            information.append(item[BIKES_AVAILABLE])
            information.append(item[DOCKS_AVAILABLE])
            information.append(has_kiosk(item))
...
def get_nearest_station(lat: float, lon: float, with_kiosk: bool,
    ...
    7486
    """
    closest_station = stations[0][ID]
    distance = 0
    closest_distance = get_lat_lon_distance(lat, lon, stations[0][LATITUDE], 
                                            stations[0][LONGITUDE])
    
    for item in stations:
        distance = get_lat_lon_distance(lat, lon, item[LATITUDE], item[LONGITUDE])
        
        if distance < closest_distance and has_kiosk(item) == with_kiosk:
            closest_station = item[ID]
...
def redistribute_bikes(stations: List["Station"]) -> int:
    ...
    True
    """
    percentage = get_total(BIKES_AVAILABLE, stations)/get_total(CAPACITY, stations)
    target = 0
    change = 0
    sum_changes = 0
    
    for item in stations:
        target = int(round(percentage*item[CAPACITY]))
        ...
        if change > 0:
            while i < change:
                return_bike(item[ID], stations)
                i = i + 1
        else:
            while i < abs(change):
                rent_bike(item[ID], stations)
                i = i + 1
